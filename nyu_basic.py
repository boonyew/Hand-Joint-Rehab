...
#    
    ...
    joints = resizeJoints(base_labels[:,idx,:].copy(),left,right,top,bottom)
...
test_sd = np.std(test_imgs)
test_imgs = [((i - test_mean) / test_sd) for i in test_imgs]
# Debug code
#
#img,label = loadDepthMap(val_dir,val_kjoints,val_bbox,val_centers,5000)
#plt.imshow(test_imgs[5000])
#for x,y,z in np.reshape(test_labels[5000],(14,3)):
#    plt.plot(x,y,color='green', marker='o')
###-------------------------------------------------------------------------------####
        
frames=5

def generate_data(imgs,gtlabels,frames,batch_size,val=False):
    """
...
#    else:
    idx_list = [i for i in range(0,end_idx) if i not in invalid_start]
...
#                while file_idx in invalid_start:
#                    file_idx = np.random.randint(0,end_idx)
...
model.fit_generator(
    generate_data(train_imgs,train_labels,frames,batch_size),
    epochs=10,
    validation_data=generate_data(test_imgs,test_labels,frames,batch_size,True),
